  ...
  return <>
    ...
    <footer id="contact" className="contact-section page-padding" aria-labelledby="contact-title">
      ...
      <div className="contact-list">{contacts.map((contact, index) => {
        const inner = <><span className="contact-number">0{index + 1}</span><span className="contact-name">{contact.label}</span><span className="contact-value">{contact.href ? contact.value ?? contact.label : text.footer.pending}</span>{contact.href ? <ArrowIcon /> : <span className="contact-empty" aria-hidden="true">—</span>}</>;
        return contact.href ? <a key={contact.kind} href={contact.href} className="contact-row" rel={contact.kind === "email" ? undefined : "noopener noreferrer"} target={contact.kind === "email" ? undefined : "_blank"}>{inner}</a> : <div key={contact.kind} className="contact-row contact-placeholder">{inner}</div>;
      })}</div>
      <div className="footer-bottom"><a href="#hero" className="wordmark" aria-label={text.footer.top}>ya<span>.</span></a><span>© 2026 {profile.name[locale]}<span className="footer-divider">/</span>{text.footer.demo}</span><a href="#hero" className="back-top">{text.footer.top}<ArrowIcon direction="up" /></a></div>
    </footer>
  </>;
}
