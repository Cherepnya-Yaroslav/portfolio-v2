...
export function ProjectEditor({ initial, current, disabled, onSaved, onDirtyChange, onBusyChange, onDelete, onToggleVisibility }: Props) {
  ...
  return <form className="admin-editor" onSubmit={submit} noValidate>
    ...
    <fieldset disabled={disabled} className="admin-fields">
      ...
      }} /><small>JPG, PNG или WebP · до 5 МБ · автоматически сжимаем до 1920 px</small></label>{file && <button type="button" className="admin-text-button" onClick={() => { setFile(null); setBlobUrl(""); }}>Отменить замену обложки</button>}{errors.image_path && <p className="admin-field-error" role="alert">{errors.image_path}</p>}</div>
